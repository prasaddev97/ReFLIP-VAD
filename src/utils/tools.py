...
def pad(feat, min_len):
    ReFLIP_length = feat.shape[0]
    if ReFLIP_length <= min_len:
       return np.pad(feat, ((0, min_len - ReFLIP_length), (0, 0)), mode='constant', constant_values=0)
    else:
       return feat
...
def process_feat(feat, length, is_random=False):
    ...
    if feat.shape[0] > length:
        ...
        return pad(feat, length), ReFLIP_length

def process_split(feat, length):
    ReFLIP_length = feat.shape[0]
    if ReFLIP_length < length:
        return pad(feat, length), ReFLIP_length
    else:
        split_num = int(ReFLIP_length / length) + 1
        ...
        return split_feat, ReFLIP_length