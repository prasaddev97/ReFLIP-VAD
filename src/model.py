...
class ReFLIPVAD(nn.Module):
    def __init__(self,
        ...
        self.ReFLIPmodel, _ = ReFLIP.load("ViT-B/16", device)
        for ReFLIP_param in self.ReFLIPmodel.parameters():
            ReFLIP_param.requires_grad = False
    ...
    def encode_textprompt(self, text):
        word_tokens = ReFLIP.tokenize(text).to(self.device)
        word_embedding = self.ReFLIPmodel.encode_token(word_tokens)
        ...
        # Reparameterize text embeddings using DistilBERT
        input_ids = self.distilbert_tokenizer.batch_encode_plus(text, padding=True, truncation=True, return_tensors='pt').input_ids.to(self.device)
        distilbert_output = self.distilbert_model(input_ids).last_hidden_state
        reparameterized_text_embeddings = distilbert_output.mean(dim=1).unsqueeze(1).expand_as(text_embeddings)

        if self.residual:
            # Skip connection: Combine original and reparameterized embeddings
            text_embeddings = text_embeddings + reparameterized_text_embeddings

        text_features = self.ReFLIPmodel.encode_text(text_embeddings, text_tokens)
